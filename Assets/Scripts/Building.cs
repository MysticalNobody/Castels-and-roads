...
public class Building : MonoBehaviour {
    ...
    private static GameObject CheckBuildingsAround(GameObject obj, int pos, bool findCastle = false) {
        ...
        if (pos == 0) {
            try {
                if (!GameObject.Find((nameInt[0] - 1) + "_" + nameInt[1]).GetComponent<TileManager>().owner.Equals(obj.GetComponent<TileManager>().owner)) {
                    return null;
                }
            } catch { }
            ...
            try {
                if (!GameObject.Find((nameInt[0]) + "_" + (nameInt[1] + 1)).GetComponent<TileManager>().owner.Equals(obj.GetComponent<TileManager>().owner)) {
                    return null;
                }
            } catch { }
